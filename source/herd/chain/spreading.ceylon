"Chain step that provides spreading capabilities.
 ...
 [[SpreadingChain]] steps require a function whose return type can be spreaded (i.o.w, satisfies [Anything*])
...
shared interface SpreadingChain<Return> satisfies
          IInvocable<Return>
        & IIterable<Return>
        & ProbingChain<Return>
        & ForcingChain<Return>
    ...
    see (`interface ISpreadable`, `function package.spread`, `function package.spreads`)
    shared SpreadingChain<NewReturn> spread<NewReturn>(NewReturn(*Return) newFunc)
...
shared interface ISpreadable<Return>
...
class Spreading<NewReturn, Return>(IInvocable<Return> prev, NewReturn(Return) func)
        ...
        satisfies SpreadingChain<NewReturn>
        given NewReturn satisfies Anything[] {}
...
shared SpreadingChain<Return> spread<Return, Arguments>(Arguments arguments, Return(Arguments) func)
        given Return satisfies Anything[]
        => object extends ChainStart<Return,Arguments>(func, arguments)
        satisfies SpreadingChain<Return> {};
...
shared SpreadingChain<Return> spreads<Return, Arguments>(Arguments arguments, Return(*Arguments) func)
        given Return satisfies Anything[]
