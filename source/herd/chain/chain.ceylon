...
shared interface IInvocable<out Return, Arguments>
{
...
class Invocable<out Return, Arguments>(Return(Arguments) func)
...
shared interface IChainable<Return, Arguments> satisfies IInvocable<Return,Arguments> {

    shared default IChainable<NewReturn,Arguments> \ithen<NewReturn>(NewReturn(Return) newFunc) => Chainable(this, newFunc);
    shared default IOptionable<NewReturn|Return,Arguments> thenOptionally<NewReturn,FuncParams>(NewReturn(FuncParams) newFunc) => Optionable(this, newFunc);
    ...
    shared default IOptionable<NewReturn|Return,Arguments> optionallyTo<NewReturn,FuncParams>(NewReturn(FuncParams) newFunc) => thenOptionally(newFunc);
...
class Chainable<Return, Arguments, PrevReturn>(IInvocable<PrevReturn,Arguments> prevCallable, Return(PrevReturn) func)
...
shared IChainable<Return,Arguments> chain<Return, Arguments>(Return(Arguments) func) => ChainStart(func);

class ChainStart<Return, Arguments>(Return(Arguments) func)
