...
shared interface ITrying<Return, Arguments> satisfies IChainable<Return|Arguments,Arguments> {}

"Basic class implementing IOptionable. Optionable actually implements the existence checking capability."
class Trying<NewReturn, Arguments, Return, FuncArgs>(IInvocable<Return> prevCallable, NewReturn(FuncArgs) func) satisfies ITrying<NewReturn|Return,Arguments> {
...
shared ITrying<Return|OtherArgs,Arguments> trying<Return, Arguments, OtherArgs>(Return(*Arguments) func, OtherArgs arguments)
        given Arguments satisfies Anything[] => TryingStart<Return|OtherArgs,Arguments,OtherArgs>(func, arguments);

class TryingStart<Return, Arguments, FuncArgs>(Return(*Arguments) func, FuncArgs arguments) satisfies ITrying<Return|FuncArgs,Arguments>
