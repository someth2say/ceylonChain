...
shared test void testChainShrink() {
    ...
    assertEquals(2, chain(0, cTtoTN).shrink(cTtoT).do(), "Conditional composition should be able to compose on callables not accepting null.");
    assertEquals(0, chain(1, cTtoTN).shrink((Null n)=>0).do(),"Conditional composition should be able to compose on callables not accepting null."); //<-Shrink behaviour
