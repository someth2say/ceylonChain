...
shared test void testChainShrink() {
    ...
    //assertEquals(null, chain(1, cTtoN).probe(cTtoT).do(), "Conditional composition should be able to compose on callables not accepting null."); //<-- Probe behaviour
    assertEquals(0, chain(1, cTtoN).probe((Null n)=>0).do(),"Conditional composition should be able to compose on callables not accepting null."); //<-Shrink behaviour
...
shared test void testShrinkMethods() {
    //to
